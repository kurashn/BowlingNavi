...
export default async function PreviewArticlePage({ params }: PageProps) {
    ...
    const article = await getWPPostById(id);

    if (!article) {
        notFound();
    }

    let processedContent = article.content.replace(/(<h2>目次[\s\S]*?<\/h2>\s*<(?:ul|ol)>[\s\S]*?<\/(?:ul|ol)>)/g, '<div class="toc-wrapper">$1</div>')
        ...
        <div className="min-h-screen bg-slate-50 pb-20 text-slate-800">
            ...
            <div className="pt-24 pb-12 lg:pt-32 lg:pb-16 bg-slate-50">
                <div className="container mx-auto max-w-4xl px-4">
                    ...
                    {/* Title */}
                    <h1 className="text-2xl font-bold leading-tight tracking-tight text-slate-900 md:text-3xl lg:text-4xl lg:leading-[1.3] mb-10">
                        {article.title}
                    </h1>
