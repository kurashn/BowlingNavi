...
export function CalendarView({ tournaments }: CalendarViewProps) {
    ...
    return (
        <div className="rounded-2xl border border-white/10 bg-slate-900/50 p-4 sm:p-6 overflow-hidden">
            {/* Header */}
            ...
            <div className="overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 sm:pb-0 custom-scrollbar">
                <div className="min-w-[600px] grid grid-cols-7 gap-px bg-white/10 rounded-xl overflow-hidden">
                    {/* Weekday Headers */}
                    {weekDays.map((day, i) => (
                        <div
                            key={day}
                            className={`bg-slate-900 p-2 sm:p-3 text-center text-xs sm:text-sm font-semibold
                    ...
                    {daysInMonth.map((day, idx) => {
                        ...
                        return (
                            <div
                                key={day.toString()}
                                className={`min-h-[90px] sm:min-h-[120px] bg-slate-900 p-1 sm:p-2 transition-colors
                                ...
                                <div className="space-y-1 overflow-y-auto max-h-[60px] sm:max-h-[80px] custom-scrollbar">
                                    {dayTournaments.map(t => (
                                        <Link key={t.id} href={`/tournaments/${t.id}`}>
                                            <div className="group flex flex-col gap-0.5 p-1 sm:p-1.5 rounded-md bg-slate-800/50 border border-white/5 hover:border-blue-500/30 hover:bg-blue-900/20 transition-all cursor-pointer">
                                                <div className="flex items-center gap-1 sm:gap-1.5">
                                                    ...
                                                    <div className="text-[10px] sm:text-xs text-slate-300 font-medium truncate group-hover:text-blue-300">
                                                        {t.title}
                                                    </div>
                                                </div>
                                                <div className="text-[9px] sm:text-[10px] text-slate-500 truncate pl-2 sm:pl-3">
                                                    {t.location}
                                                </div>
                                            </div>
                                        </Link>
